...
export const hungarianMatcher: Matcher = (left, right, compareFunc) => {
    ...
    const cost = left.map((l) => right.map((r) => compareFunc(l, r, true)[1]));
    ...
    rawPairs.forEach(([lIndex, rIndex]) => {
        ...
        const costPotentialRatio = cost / (countSubElements(leftElement.raw) + countSubElements(rightElement.raw));
